    ...
    <Layout>
      ...
      <section className="min-h-screen pt-10 px-4 sm:px-8 lg:px-16 bg-primary-dark text-neutral-text">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl font-extrabold mb-10 border-b-4 border-accent pb-2 inline-block">My Projects</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {PORTFOLIO_DATA.projects.map(project => (
              <div key={project.id} className="bg-surface-light rounded-xl shadow-xl hover:shadow-2xl transition-shadow duration-300 overflow-hidden group text-primary-dark flex flex-col">
                <div className="p-6 flex flex-col h-full">
                  <h3 className="text-2xl font-bold mb-2 group-hover:text-accent transition-colors">
                    {project.title}
                  </h3>
                  <p className="text-primary-dark/80 mb-4 grow">{project.description}</p>
